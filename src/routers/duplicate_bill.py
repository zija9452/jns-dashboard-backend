...
async def search_duplicate_bills(
    ...
    scope: str = Query("general", description="'general' returns walk-in + customer invoices; 'warehouse' returns only warehouse invoices"),
    ...
    """
    if scope not in ("general", "warehouse"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid scope. Must be 'general' or 'warehouse'"
        )
